    ...
    <div v-if="visibility.overlay"
      ...
      <transition :name="transition">
        <div v-if="visibility.modal"
             ...
             @touchstart.stop>
          <slot/>
          <resizer v-if="resizable && !isAutoHeight && mode == 'default'"
                   :min-width="minWidth"
...
<script>
  ...
  export default {
    ...
    props: {
      ...
      customWidth: { // means that the width will be set by css class, for example
        type: [Boolean],
        default: false,
      },
      pivotX: {
    ...
    computed: {
      ...
      position () {
        ...
        let lRange = inRange(0, maxLeft, left)
        let tRange = inRange(0, maxTop, top)
        if (lRange != left) this.shift.left = lRange - pivotX * maxLeft
        if (tRange != top) this.shift.top = tRange - pivotY * maxTop
        // this.shift.left = (lRange != left) ? lRange : left
        // this.shift.top = (tRange != top) ? tRange : top
       ...
       */
      modalClass () {
        let classes = ['v--modal-box', this.classes]
        this.animated && classes.push('animated')
        if ( this.mode == 'pull-right' || this.mode == 'pull-left' ) classes.push('docked')
        return classes
      },
      /**
      ...
      modalStyle () {
        let width = this.customWidth
            ? 'auto'
            : (this.trueModalWidth + 'px')
        let height = this.isAutoHeight
            ? 'auto'
            : (this.trueModalHeight + 'px')

        return {
          top: this.position.top + 'px',
          left: this.customWidth ? 'auto' : this.position.left + 'px',
          width: width,
          height: height
        }
      },

      alternativeModalStyle () {
        let left = (this.mode == 'pull-left') ? '0px' : 'auto'
        let right = (this.mode == 'pull-right') ? '0px' : 'auto'

        // dockedWidth

        let dockedWidth = parseNumber(this.dockedWidth)

        return {
          top: '0px',
          left, right,
          width: dockedWidth.value + dockedWidth.type,
          height: '100vh'
        }
      },

      finalStyle () {
        return ( this.mode == 'pull-right' || this.mode == 'pull-left' )
          ? this.alternativeModalStyle
          : this.modalStyle
      }
    },
    methods: {
      ...
      onWindowResize () {
        // console.log('window resize', window, window.innerHeight)
...
<style>
  ...
  .v--modal-overlay .v--modal-box.animated {
  ...
  .v--modal-overlay.disabled .v--modal-box {
    position: fixed;
  }
